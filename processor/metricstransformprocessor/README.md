...
| Delete metrics                | delete metrics with name `system.cpu.usage_time`                                                |
...
transforms:
  ...
  - include: <metric_name>
    ...
    # action specifies if the operations (specified below) are performed on metrics in place (update/delete), on an inserted clone (insert), or on a new combined metric (combine)
    action: {update, insert, combine, delete}
