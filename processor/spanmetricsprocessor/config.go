...
type Config struct {
	...
	// Transforms takes in a list of Actions to perform on metrics generated by span metrics processor.
	// currently there is only one action, the rename action. This allows a user to specify metrics with certain attributes
...
// Transform defines the new metric names and the attributes to match on in order for the rename to occur
...
func (t Transform) attributesMatched(attributesOnMetric *pdata.AttributeMap) bool {
	...
	// check if all attributes specified to match on in transform exist on attributes that will be attached to metric
